...
def play_game(gamectrl):
    ...
    while 1:
        ...
        if move < 0:
            print('NO VALID MOVE')
            break
        print("%010.6f: Score %d, Move %d: %s" % (time.time() - start,
                                                  gamectrl.get_score(),
                                                  moveno, movename(move)))
...
def parse_args(argv):
    ...
    parser.add_argument('-p', '--port', help="Port number to control on "
                             "(default: 32000 for Firefox, 9222 for Chrome)",
                        type=int)
    parser.add_argument('-b', '--browser', help="Browser you're using. "
                                                "Only Firefox with the Remote Control extension, and Chrome with remote debugging (default), are supported right now.", default='chrome', choices=('firefox', 'chrome'))
    parser.add_argument('-k', '--ctrlmode',
                        help="Control mode to use. "
                             "If the browser control doesn't seem to work, "
                             "try changing this.",
                        default='hybrid',
